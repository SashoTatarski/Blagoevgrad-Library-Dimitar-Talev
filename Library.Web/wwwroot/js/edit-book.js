$('#add-author-edit').click(function () {
    $('#create-author-modal-edit').modal('show');
    console.log('Success');
});

$('#create-author-edit').click(function () {
    const authorNameValue = $('#new-author-name-edit').val();
    ...
    $.ajax({
        ...
        success: function (response) {
            $('#create-author-modal-edit').modal('hide');

            const option = $(`<option value="${response.id}">${response.name}</option>`);

            $('#authors-selectlist-edit').append(option);
